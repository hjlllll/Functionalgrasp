...
class ModelTrainer:
    ...
    def train(self, net, training_loader, val_loader, config, debug=False):
        ...
        for epoch in range(config.max_epoch):
            ...
            for ib, batch in enumerate(training_loader):
                ...
                # # 角度loss
                angle_lower = torch.tensor([ 0., -20.,  0.,   0., -20.,  0.,   0., -20.,  0.,   0., -20.,  0.,   0., -60.,  0., -12, -30.,  0.]).cuda() / 90.0  # * 1.5708 / 1.5708
                angle_upper = torch.tensor([45.,  20., 90., 180.,  20., 90., 180.,  20., 90., 180.,  20., 90., 180.,  60., 70.,  12., 30., 90.]).cuda() / 90.0  # * 1.5708 / 1.5708
                angle_lower_pair = torch.zeros([2, outputs_a.reshape(-1).shape[0]]).cuda() #[2,12*18]  outputs_a = [12,18]
                angle_upper_pair = torch.zeros([2, outputs_a.reshape(-1).shape[0]]).cuda()
                angle_lower_pair[0] = angle_lower.repeat(outputs_a.shape[0]) - outputs_a.reshape(-1) #[12*18] - [12*18]
                ...
                # # 手自碰撞约束loss_handself
                # 中指尖与其他指尖
                hand_self_distance0 = ((outputs_FK[:, [16]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [27, 21, 11, 6]].shape[1], -1)
                                - outputs_FK[:, [27, 21, 11, 6]].unsqueeze(1).expand(-1, outputs_FK[:, [16]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0],-1)
                # 拇指指尖+无名指指尖 vs 食指指尖+小拇指指尖
                hand_self_distance1 = ((outputs_FK[:, [27, 11]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [21, 6]].shape[1], -1)
                                - outputs_FK[:, [21, 6]].unsqueeze(1).expand(-1, outputs_FK[:, [27, 11]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0],-1)
                # 拇指指尖+小拇指指尖 vs 食指指尖+无名指指尖
                hand_self_distance2 = ((outputs_FK[:, [27, 6]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [21, 11]].shape[1], -1)
                                - outputs_FK[:, [21, 11]].unsqueeze(1).expand(-1, outputs_FK[:, [27, 6]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0],-1)
                # 小拇指2\3关节 vs 其他指尖
                # hand_self_distance3 = ((outputs_FK[:, [26, 24]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [21, 20, 16, 11, 6]].shape[1], -1)
                #                 - outputs_FK[:, [21, 20, 16, 11, 6]].unsqueeze(1).expand(-1, outputs_FK[:, [26, 24]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0], -1)
                hand_self_distance3 = ((outputs_FK[:, [5, 4]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [27, 21, 16, 11, 6]].shape[1], -1)
                                - outputs_FK[:, [27, 21, 16, 11, 6]].unsqueeze(1).expand(-1, outputs_FK[:, [5, 4]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0], -1)
                hand_self_distance4 = ((outputs_FK[:, [26,24]].unsqueeze(2).expand(-1, -1, outputs_FK[:, [21, 20, 16, 11, 6]].shape[1], -1)
                                - outputs_FK[:, [21, 20, 16, 11, 6]].unsqueeze(1).expand(-1, outputs_FK[:, [26,24]].shape[1], -1, -1)) ** 2).sum(-1).sqrt().reshape(outputs_FK.shape[0], -1)
                ...
                hand_self_distance = torch.cat([hand_self_distance0, hand_self_distance1, hand_self_distance2, hand_self_distance3,hand_self_distance4], 1).reshape(-1)/30
                ...
                batch_dis_away = batch_distance[:, :, [27, 26, 24, 23, 21, 20, 19, 18, 16, 15, 14, 13, 11, 10, 9, 8, 6, 5, 4, 3, 0]] * batch_features_away  # 20210707多了个关节，序号变动
                ...
                loss_away = torch.tensor([1,1,1,2,1,1,1,2,1,1,1,2,1,1,1,2,1,1,1,2,5]).cuda() * loss_away
                loss_away[loss_away <= 0] = 0
                loss_away = loss_away.sum() / batch_dis_away.shape[0]

                loss = 0.1 * loss_close  + 1.0* loss_away + 10 * loss_angles + 10 * loss_handself   #  20210105
                # loss = loss_angles1
                acc = loss
                t += [time.time()]
            ...
            # if self.epoch == 0:
                ...
                for i in range(grip_r.shape[0]):
                    ...
                    write_xml(obj_name[grip_idx[i]], grip_r[i], grip_t[i], grip_a[i], path=results_xml_path + '/train/epoch{}_{}_{}.xml'.format(self.epoch, grip_idx[i], i), mode='train', rs=(21, 'real'))
    ...
    def object_grasping_validation(self, net, val_loader, config, results_xml_path, epoch):
        ...
        for batch in val_loader:
            ...
            if batch.labels.shape[1]>1:
                outputs_ = outputs.clone().detach().unsqueeze(1)
                label_idx = torch.sqrt(torch.sum(torch.square(outputs_ - batch.labels[:, :, 0:25]), 2)).argmin(dim=1) # 取最小的一个
        ...
        if epoch % 20 == 10: # and epoch != 0
            ...
            for i in range(val_grip_r.shape[0]):
                # print('22222222222222222222222222222222222222222222', grip_a[i].shape)
                # print('22222222222222222222222222222222222222222222', grip_a[i][17])
                write_xml(val_obj_name[val_grip_idx[i]], val_grip_r[i], val_grip_t[i], val_grip_a[i], path=join(grip_path, 'epoch{}_{}_{}_val.xml'.format(epoch, val_grip_idx[i], i)), mode='val', rs=(21, 'real'))
