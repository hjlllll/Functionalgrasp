...
class PointCloudDataset(Dataset):
    ...
    def augmentation_grasp(self, points, label, is_norm=True, verbose=False, is_aug=True):
        """Implementation of an augmentation transform for point clouds."""
        ...
        if is_aug:
            ...
            tran = (np.random.randn(points.shape[1]) * self.config.augment_trans).astype(np.float32)
            ...
            augmented_label[:, 4:7] = np.sum(np.expand_dims(label[:, 4:7], 2) * R, axis=1) + tran  # 写的不对，因为label的旋转没有跟着变，不太好写所以先不写了，也暂时不使用数据扩充了
            # augmented_label = self.aug_label(label, R, trans)  # # ?????????????????????????????????????????==================================================??????????????????????????????

        else:
            tran = np.zeros(points.shape[1]).astype(np.float32)
            augmented_points = points
            augmented_label = label

        if is_norm:
            # augmented_label[:,  :4] = augmented_label[:,  :4]
            augmented_label[:, 4:7] = augmented_label[:, 4:7] * 5.0
            augmented_label[:, 7:] = augmented_label[:, 7:] / 1.5708


        if verbose:
            test_p = [np.vstack([points, augmented_points])]
            test_l = [np.hstack([points[:, 2]*0, augmented_points[:, 2]*0+1])]
            show_ModelNet_examples(test_p, test_p, test_l)

        return augmented_points, augmented_label, tran, R
    ...
    def segmentation_inputs(self,
        ...
        li += [stacked_features, labels.astype(np.int64)]
